...
Describe 'update-editorconfig-csharp convention' {
	It 'creates the published C# editorconfig source file from markdown' {
		...
		try {
			...
			Remove-Item -LiteralPath (Join-Path $testDirectory 'conventions\editorconfig-csharp\.editorconfig') -Force -ErrorAction SilentlyContinue
			...
			$generatedPath = Join-Path $testDirectory 'conventions\editorconfig-csharp\.editorconfig'
			$expectedPath = Join-Path ([System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..\..')) ) 'conventions\editorconfig-csharp\.editorconfig'
