...
The file is created or overwritten with a GitHub workflow called **Apply Repository Conventions**, which uses the [repo-conventions reusable workflow](../../.github/workflows/repo-conventions-call.yml) to apply repository conventions every weekday with [repo-conventions](https://github.com/Faithlife/RepoConventions). When run manually, the workflow accepts optional space-delimited convention names to add and apply in the generated pull request. If an existing workflow is already up to date, the convention preserves the workflow's existing scheduled minute instead of rewriting it only because a new random minute would have been selected.
