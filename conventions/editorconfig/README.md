...
If `.editorconfig` does not exist, the convention seeds it with `root = true`, then adds a blank line and the managed section. After the convention changes `.editorconfig`, it runs Copilot with build-validation instructions so any follow-up fixes stay in the working tree. If the named section already exists, the convention replaces only that section. Other `.editorconfig` content is preserved.
