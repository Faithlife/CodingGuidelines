...
function InvokeLineBasedConfigConvention {
	param(
		[Parameter(Mandatory = $true)]
...
Describe 'line-based-config convention' {
	It 'creates a repository-root-relative file and is idempotent' {
		$testDirectory = NewTestDirectory

		try {
			$output = InvokeLineBasedConfigConvention -TestDirectory $testDirectory -Settings @{ path = '/.gitignore'; entries = @('bin/', 'obj/') }
			...
			$secondOutput = InvokeLineBasedConfigConvention -TestDirectory $testDirectory -Settings @{ path = '/.gitignore'; entries = @('bin/', 'obj/') }
	...
	It 'resolves relative paths from the repository root' {
		...
		try {
			InvokeLineBasedConfigConvention -TestDirectory $testDirectory -Settings @{ path = 'workflows/ci.yml'; entries = @('name: CI') }
	...
	It 'appends only entries that are not already present' {
		...
		try {
			...
			InvokeLineBasedConfigConvention -TestDirectory $testDirectory -Settings @{ path = '/.editorconfig'; entries = @('root = true', '[*]') }
	...
	It 'does not create the target file when the configured entries list is empty' {
		...
		try {
			...
			$output = InvokeLineBasedConfigConvention -TestDirectory $testDirectory -Settings @{ path = '.gitignore'; entries = @() }
	...
	It 'rejects entries that contain newlines' {
		...
		try {
			{ InvokeLineBasedConfigConvention -TestDirectory $testDirectory -Settings @{ path = '/.gitignore'; entries = @("bin/`nobj/") } } | Should Throw "Each entry in 'entries' must be a single line."
		}
		finally {
			Remove-Item -LiteralPath $testDirectory -Recurse -Force
		}
